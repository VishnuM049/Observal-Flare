        ...
        <tbody>
          {sites.map((site) => (
            ...
            >
              ...
              <td className="px-4 py-3" style={{ color: "var(--color-ink-light)" }}>
                {formatDailyCost(estimateDailyCost(site.instance_size, site.sleep_mode, site.sleep_at_hour, site.wake_at_hour, site.idle_timeout_minutes, site.cloud_provider))}
              </td>
              <td className="px-4 py-3">
        ...
        <tfoot style={{ borderTop: "1px solid var(--color-border)" }}>
          <tr style={{ backgroundColor: "var(--color-cream)" }}>
            ...
            <td className="px-4 py-3 font-medium" style={{ color: "var(--color-ink-light)" }}>
              {formatDailyCost(sites.reduce((sum, s) => sum + estimateDailyCost(s.instance_size, s.sleep_mode, s.sleep_at_hour, s.wake_at_hour, s.idle_timeout_minutes, s.cloud_provider), 0))}
            </td>
            <td colSpan={2}></td>
          </tr>
        </tfoot>
